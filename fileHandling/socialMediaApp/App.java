...
public class App {
    ...
    File file = new File("socialMediaApp.txt");
    ...
    public void loginAccount(){
        ...
        for (CreateAccount users : accountList) {
            if (users.username.equalsIgnoreCase(user) && users.password.equalsIgnoreCase(pascode)) {
                currentUser = new Login(user, pascode);
    ...
    void saveUsers() {
        try (BufferedWriter bufferedWriter = new BufferedWriter(new FileWriter(file))) {
            for (CreateAccount accounts : accountList) {
                bufferedWriter.write(accounts.userId + "," + accounts.username + "," + accounts.email + "," + accounts.contact + "," + accounts.password);
                bufferedWriter.newLine();
                for (Post post : accounts.posts) {
                    bufferedWriter.write(post.postId + "," + post.content);
                    bufferedWriter.newLine();
                    for (Comment comment : post.comments) {
                        bufferedWriter.write(comment.commentId + "," + comment.text);
                        bufferedWriter.newLine();
                    }
                }
            }
        ...
        void loadUsers(){
            try(BufferedReader bufferedReader = new BufferedReader(new FileReader(file))) {
                ...
                while ((line = bufferedReader.readLine()) != null){
                    ...
                    CreateAccount accounts = new CreateAccount(userId,username,email,contact,password);
                    while ((line = bufferedReader.readLine()) != null && line.startsWith("  ")){
                        ...
                        while ((line = bufferedReader.readLine()) != null && line.startsWith("    ")){
                            String[] commmentData = line.trim().split(",");
                            String commentId = commmentData[0];
                            String text = commmentData[1];

                            post.comments.add(new Comment(commentId,text));
                        }
                        accounts.posts.add(post);
                    }
